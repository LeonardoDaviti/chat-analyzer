...
# --------------------------------------------------------------------------- #
# PWA: manifest, service worker, icons (M4.1)
#
# Everything here is generated with the standard library only (no new deps, no
# network) and is served ONLY over http(s) by this launcher. When the dashboard
# is opened directly over file://, the page's manifest <link> 404s harmlessly and
# its service-worker registration is guarded off — the phone/PWA surface simply
# does not exist there. Analysis always runs on this computer; a phone that
# installs the PWA is a VIEWER of the dashboard served over the LAN.
# --------------------------------------------------------------------------- #
# Cache name is derived from APP_VERSION so a new release busts the old cache.
SW_CACHE_NAME = f"chat-analyzer-v{APP_VERSION}"
...
def pwa_manifest() -> dict:
    ...
    return {
        ...
        "description": "Private, local-first chat analytics dashboard. "
                       "Analysis runs on your computer; this installs a phone "
                       "viewer of the dashboard served over your network.",
        "id": "/",
        "start_url": "/",
        "scope": "/",
        "display": "standalone",
...
def service_worker_js() -> str:
    """The /sw.js service worker source.

    Cache-first for the app shell + /assets/* (instant repeat loads, offline
    viewing); network-first for /data/* so a freshly rebuilt analysis always
    wins. skipWaiting + clients.claim so an update takes over immediately, and
    a version-derived cache name (SW_CACHE_NAME) so old caches are purged on
    activate when APP_VERSION changes.
    """
    return (
        "// Auto-generated by launcher.py — do not edit.\n"
        f"const CACHE = {json.dumps(SW_CACHE_NAME)};\n"
        "const SHELL = ['/', '/index.html', '/echarts.min.js',\n"
        ...
        "    // network-first: fresh analysis wins, cache is the offline fallback.\n"
...
def _render_icon_png(size: int) -> bytes:
    """Render a simple app icon PNG (stdlib only): a rounded dark square with an
    accent bar-chart glyph (📊-like). No fonts, no dependencies."""
    bg = (24, 27, 31)          # --bg
    accent = (77, 182, 172)    # --a
    grey = (158, 158, 158)     # --b
    radius = size * 0.18
    ...
    gap = inner * 0.08
    bw = (inner - gap * (n - 1)) / n
    heights = [0.45, 0.72, 0.55, 0.95]
    colors = [accent, grey, accent, accent]
    baseline = size - margin
    bars = []
    for i in range(n):
        x0 = margin + i * (bw + gap)
        y0 = baseline - inner * heights[i]
        bars.append((x0, x0 + bw, y0, baseline, colors[i]))
    ...
    for y in range(size):
        ...
        for x in range(size):
            cx = x if x < size - 1 - x else size - 1 - x
            cy = y if y < size - 1 - y else size - 1 - y
...
def pwa_icon_png(size: int) -> bytes:
    """Memoized PNG icon at the requested square size."""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if route == "/manifest.webmanifest":
            self._send(200, json.dumps(pwa_manifest()).encode("utf-8"),
                       "application/manifest+json; charset=utf-8")
            return
        if route == "/sw.js":
            # Served from the root so its scope covers the whole app.
            self._send(200, service_worker_js().encode("utf-8"),
                       "application/javascript; charset=utf-8",
                       extra={"Service-Worker-Allowed": "/",
                              "Cache-Control": "no-cache"})
            return
        if route in ("/app-icon-192.png", "/app-icon-512.png"):
            size = 512 if "512" in route else 192
            self._send(200, pwa_icon_png(size), "image/png")
            return
        if route == "/":
            if manifest_ready():
...
# Server bootstrap
# --------------------------------------------------------------------------- #
def _bind_server(host: str = None, port: int = None) -> ThreadingHTTPServer:
    """Bind the preferred port on ``host``, else fall back to an ephemeral one."""
...
def _parse_args(argv=None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        prog="chat-analyzer",
        description="Chat Analyzer launcher — a local dashboard server.")
    ap.add_argument(
        "--host", default=HOST,
        help="interface to bind (default 127.0.0.1, localhost only). "
             "Use 0.0.0.0 to also reach the dashboard from your phone on the "
             "same network — only on a trusted network.")
    ap.add_argument("--port", type=int, default=PORT,
                    help=f"preferred port (default {PORT})")
    return ap.parse_args(argv)


def _lan_ip() -> str:
    """Best-effort LAN IP for the phone hint (no traffic actually sent)."""
...
def main(argv=None) -> None:
    ...
    httpd = _bind_server(host=args.host, port=args.port)
    ...
    if host in ("0.0.0.0", "::"):
        # Bound to all interfaces on purpose (--host): print the LAN address a
        # phone would use, plus a one-line security note.
        print(f"On your phone (same Wi-Fi): http://{_lan_ip()}:{port}/")
        print("NOTE: --host exposes the dashboard to your whole network. "
